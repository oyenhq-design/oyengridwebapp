    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-200">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="p-8 rounded-3xl bg-slate-900/90 border border-slate-800 relative overflow-hidden shadow-xl group">
            ...
            <h3 className="text-xl font-extrabold text-slate-100 mb-2">Design Systems & Component Tokens</h3>
            ...
            <button
              onClick={() => setActiveTab('learning')}
              className="px-6 py-3 rounded-2xl bg-amber-400 hover:bg-amber-300 text-slate-950 text-xs font-black flex items-center gap-2 transition-all shadow-md shadow-amber-400/10"
            >
          ...
          <div className="space-y-4">
            ...
            <div className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800 flex items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="px-3 py-2 rounded-xl bg-amber-400/10 border border-amber-400/20 text-amber-400 font-extrabold text-xs text-center shrink-0">
                  10:00 AM
                </div>
                <div>
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className="px-2 py-0.5 rounded text-[9px] font-extrabold bg-emerald-500/20 text-emerald-400">LIVE SESSION</span>
                    <h4 className="text-xs font-extrabold text-slate-100">Design Systems & Component Tokens</h4>
                  </div>
                  <p className="text-[11px] text-slate-400">Facilitator: <span className="text-slate-300 font-semibold">Sarah Ahmed</span></p>
                </div>
              </div>

              <button
                onClick={() => setActiveTab('sessions')}
                className="px-4 py-2 rounded-xl bg-amber-400 hover:bg-amber-300 text-slate-950 font-extrabold text-xs shadow-md shadow-amber-400/10 transition-all shrink-0"
              >
                Join Session
              </button>
            </div>
          ...
          <div className="p-6 rounded-3xl bg-slate-900/80 border border-slate-800 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-extrabold text-slate-200 uppercase tracking-wider flex items-center gap-2">
                <GraduationCap size={16} className="text-amber-400" /> Programme Roadmap Journey
              </h3>
          ...
          <div className="space-y-3">
            <h3 className="text-sm font-extrabold text-slate-200 uppercase tracking-wider flex items-center gap-2">
              <Megaphone size={16} className="text-amber-400" /> Workspace Announcements
            </h3>

            <div className="space-y-2">
              {announcements.map(ann => (
                <div key={ann.id} className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800/80 flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">{ann.detail}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-8">
          ...
          {/* Today's Actionable Goal Checklist (Replaces analytics) */}
          <div className="p-6 rounded-3xl bg-slate-900/90 border border-slate-800 space-y-4 shadow-xl">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-extrabold text-slate-100 uppercase tracking-wider flex items-center gap-2">
                <CheckCircle2 size={16} className="text-amber-400" /> Today's Action Goal
              </h3>
          ...
          <div className="p-6 rounded-3xl bg-gradient-to-b from-slate-900 via-slate-900 to-amber-950/30 border border-amber-400/20 shadow-xl space-y-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-sm font-extrabold text-slate-100">OYEN AI Assistant</h3>
                <p className="text-[10px] text-amber-400/80 font-medium">Instant study assistant</p>
              </div>
            </div>
            ...
            <form onSubmit={(e) => { e.preventDefault(); setActiveTab('ai-assistant'); }} className="relative">
              <input
                ...
                className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-3 pr-9 py-2.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-amber-400/50"
              />
              <button type="submit" className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-lg bg-amber-400 text-slate-950 font-bold">
                <ArrowRight size={12} />
              </button>
            </form>
            ...
            <div className="space-y-1.5">
              ...
              {['Explain Flexbox & Grid', 'Summarize Week 4 notes', 'Help with assignment rubric'].map((prompt, i) => (
                <button
                  ...
                  className="w-full text-left p-2 rounded-xl bg-slate-950/60 border border-slate-800 hover:border-amber-400/40 text-[11px] text-slate-400 hover:text-amber-400 transition-all flex items-center justify-between"
                >
                  <span>• {prompt}</span>
          ...
          <div className="p-6 rounded-3xl bg-slate-900/80 border border-slate-800 space-y-4">
            <h3 className="text-sm font-extrabold text-slate-200 uppercase tracking-wider flex items-center gap-2">
              <History size={16} className="text-amber-400" /> Recent Activity Log
            </h3>

            <div className="space-y-2.5">
              {recentActivity.map(act => (
                <div key={act.id} className="flex items-center justify-between p-2.5 rounded-xl bg-slate-950/60 border border-slate-800/80 text-xs">
                  <div className="flex items-center gap-2">
